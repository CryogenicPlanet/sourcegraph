...
func subRepoFilterFunc(ctx context.Context, checker authz.SubRepoPermissionChecker) run.EventTransformer {
	...
	return func(e streaming.SearchEvent) (streaming.SearchEvent, error) {
		...
		// user so we'll return a generic user and log an error summary.
		if errs.Len() == 0 {
			return filtered, nil
		}
		errs.ErrorFormat = func(i []error) string {
			return fmt.Sprintf("sub-repo permissions. %d errors. Example: %s", errs.Len(), i[0].Error())
		}
